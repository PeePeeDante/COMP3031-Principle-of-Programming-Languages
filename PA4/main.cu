 ...
 */
int read_file(std::string filename, int* &c_c, int* &c_n, int* &c_h, int* &n_o,
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < c_c_size; i++) {
        std::cout << c_c[i] << "-" << c_c[i + c_c_size] << std::endl;
    }
    std::cout << "c-n:" << std::endl;
    for (int i = 0; i < c_n_size; i++) {
        std::cout << c_n[i] << "-" << c_n[i + c_n_size] << std::endl;
    }
    std::cout << "c-h:" << std::endl;
    for (int i = 0; i < c_h_size; i++) {
        std::cout << c_h[i] << "-" << c_h[i + c_h_size] << std::endl;
    }
    std::cout << "n-o:" << std::endl;
    for (int i = 0; i < n_o_size; i++) {
        std::cout << n_o[i] << "-" << n_o[i + n_o_size] << std::endl;
    }
#endif
    ...
    tnt_counting(num_blocks_per_grid, num_threads_per_block, 
            c_c, c_n, c_h, n_o, 
            c_c_size, c_n_size, c_h_size, n_o_size,
    ...
    for (int i = 0; i < final_result_size; i++) {
        for (int j = 0; j < NUM_TNT_VERTICES; j++) {
    ...
    fprintf(stderr, "Elapsed Time: %.9lf s\n", std::chrono::duration_cast<
        std::chrono::nanoseconds>(t_end - t_start).count() / pow(10, 9));
